...
async function run() {
    console.log('Starting Phase 63E.3 Smoke Test: Service Security / Interactivity End-to-End Regression\n');
    ...
    if (fs.existsSync(WORKER_REPORT_PATH)) {
        ...
        console.log(`[TEST] Loaded ${workerScenarios.length} scenarios from Worker 63E.2 report.`);
    } else {
        console.log('[TEST] Worker 63E.2 report not found — using SYNTHETIC_POLICY_FALLBACK scenarios.');
        workerScenarios = [
    ...
    // 1. Capability contract regression — Phase 63 capabilities still exposed under pdf_security_interactivity
    try {
        ...
        assert.ok(flattenAnnotations.requires_human_review === true, 'FLATTEN_ANNOTATIONS requires_human_review should remain true');
        assert.ok(flattenAnnotations.production_safe === false, 'FLATTEN_ANNOTATIONS production_safe should remain false');
        assert.ok(flattenAnnotations.visually_sensitive === true, 'FLATTEN_ANNOTATIONS visually_sensitive should remain true');
        assert.ok(flattenAnnotations.destructive === true, 'FLATTEN_ANNOTATIONS destructive should remain true');

        const flattenForms = getCap('FLATTEN_FORMS');
        assert.ok(flattenForms.requires_human_review === true, 'FLATTEN_FORMS requires_human_review should remain true');
        assert.ok(flattenForms.production_safe === false, 'FLATTEN_FORMS production_safe should remain false');
        assert.ok(flattenForms.visually_sensitive === true, 'FLATTEN_FORMS visually_sensitive should remain true');
        assert.ok(flattenForms.destructive === true, 'FLATTEN_FORMS destructive should remain true');

        const stripJs = getCap('STRIP_JAVASCRIPT');
        assert.ok(stripJs.security_sensitive === true, 'STRIP_JAVASCRIPT security_sensitive should remain true');

        pass('Capability Contract Regression — pdf_security_interactivity capabilities (end-to-end)');
    } catch (e) { fail('Capability Contract Regression — pdf_security_interactivity capabilities (end-to-end)', e); }
    ...
    for (const ws of workerScenarios) {
        ...
        const jobId = `e2e_regression_${idx}_${scenarioName.replace(/[^A-Za-z0-9]+/g, '_').slice(0, 50)}${jobIdSuffix}`;
        ...
        try {
            // 2a. FixAuditNormalizer preservation of security_interactivity_governance and evidence (root + delta_report)
            const v2Audit = {
                version: '2.0',
            ...
            // 3. certified.pdf downgrade when review_required=true / certified_pdf_allowed=false (end-to-end)
            if (isCertCase && gov.certified_pdf_allowed === false) {
            ...
            // 4. Standards overclaim protection: security/interactivity fixes never produce compliance claims end-to-end
            ...
            // 6. Evidence preservation regression (end-to-end from Engine through Worker into Service)
            ...
            pass(`E2E scenario passthrough: ${scenarioName}`);
        } catch (e) {
            fail(`E2E scenario passthrough: ${scenarioName}`, e);
        }
    }
    ...
    fs.writeFileSync(path.join(REPORTS_DIR, 'phase63e_service_security_interactivity_regression.json'), JSON.stringify(reportData, null, 2));

    let md = `# Phase 63E.3 — Service Security / Interactivity End-to-End Regression\n\n`;
    ...
    fs.writeFileSync(path.join(REPORTS_DIR, 'phase63e_service_security_interactivity_regression.md'), md);
